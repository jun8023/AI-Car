...
__declspec( dllexport ) void change(uint32 date[X_MAX][Y_MAX], unsigned int hang, unsigned int lie,unsigned char white,unsigned char dark)
{
                   ...
                   for(left_i=right_i=start;!(((left_flag==1)&&(right_flag==1))&&(left_i>=end)&&(right_i>=end));)   //先扫前几行，确定有效边界
                    {
                        left=0;right=Y_MAX;
					...
					for(left_i,right_i,left_j=left_line[left_dot-1][1],right_j=right_line[right_dot-1][1];left_end_flag||right_end_flag;)
					{
						...
						if(change_right&change_left)//确定扫描方式
						{
							...
							if((date[left_line[left_dot-1][0]+1][left_line[left_dot-1][1]-1]==254)&&(date[left_line[left_dot-1][0]+1][left_line[left_dot-1][1]]==254))
							{
								left_l_dir=0;//向减小方向扫
							}
							else left_l_dir=1;//向增大方向扫
							if((date[right_line[right_dot-1][0]+1][right_line[right_dot-1][1]-1]==254)&&(date[right_line[right_dot-1][0]+1][right_line[right_dot-1][1]]==254))
							{
						...
						case 3:{
									...
									while(!ones_ok)//判断补线是否成功
									{
										...
										{
											left_i=(left_line[left_dot-1][0]+right_line[right_dot-1][0])/2+10;//往后先扫10行
											left_j=(left_line[left_dot-1][1]+right_line[right_dot-1][1])/2;
